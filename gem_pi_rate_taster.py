"""Dieses Programm ist ein Spiel, bei dem man als Pirat Diamanten einsammeln muss.
    ...
    im Fortgeschrittenen-Praktikum Physik an der UHH geschaffen. Diese Version kann sowohl mit
    der Tastatur als auch mit Tastern gesteuert werden. Um das Spiel ohne GPIO-Taster zu spielen
    muss die Version 'gem_pi_rate.py' verwendet werden.
...
class Spieler(pyglet.sprite.Sprite):
    ...
    def update(self, dt, pins):
        ...
        Args:
            ...
            pins(tuple): Boolean Tuple, welche pins ein Signal liefern
        """
        self.x += self.velocity_x * dt
        self.y += self.velocity_y * dt
        self.velocity_x += (pins[3]- pins[2])
        self.velocity_y += (pins[0] - pins[1])
...
def general_update(dt, obj):
    """Allgemeine Updatefunktion mit variablen Update-Objekt. Die Update Funktion für Pyglet darf
        nur einen Parameter annehmen. Nimmt die Wert an 4 Input-Pins auf.

    Args:
        dt (float): Länge des Zeitschritts
        obj (object): Objekt mit Update-Methode
    """
    pins_input = (GPIO.input(18), GPIO.input(22), GPIO.input(27), GPIO.input(17))#hoch, runter, links, rechts,
    obj.update(dt, pins_input)
